...
func Register(registry *repositoryregistry.Registry) error {
	for _, backend := range []repositoryregistry.Backend{repositoryregistry.BackendSQLite, repositoryregistry.BackendPostgres} {
		if err := registry.Register(repositoryregistry.NameInstrument, backend, createSQLRepository); err != nil {
			return err
		}
	}
	return registry.Register(repositoryregistry.NameInstrument, repositoryregistry.BackendMongoDB, createMongoRepository)
}
