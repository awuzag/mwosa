...
func Register(registry *repositoryregistry.Registry) error {
	for _, backend := range []repositoryregistry.Backend{repositoryregistry.BackendSQLite, repositoryregistry.BackendPostgres} {
		if err := registry.Register(repositoryregistry.NameIndexBar, backend, createSQLRepositories); err != nil {
			return err
		}
	}
	return registry.Register(repositoryregistry.NameIndexBar, repositoryregistry.BackendMongoDB, createMongoRepositories)
}
