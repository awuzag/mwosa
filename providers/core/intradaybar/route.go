...
type Router interface {
	RouteIntradayBars(ctx context.Context, input RouteInput) (Fetcher, error)
	PlanIntradayBars(ctx context.Context, input RouteInput) (RoutePlan, error)
}
...
func (r routeAdapter) RouteIntradayBars(ctx context.Context, input RouteInput) (Fetcher, error) {
	errb := oops.In("intradaybar_router").With("provider", input.ProviderID, "market", input.Market, "security_type", input.SecurityType, "symbol", input.Symbol)
	...
	if !ok {
		return nil, errb.With("provider", candidate.Provider.ID).New("routed intradaybar implementation does not satisfy Fetcher")
	}
	return fetcher, nil
}

func (r routeAdapter) PlanIntradayBars(ctx context.Context, input RouteInput) (RoutePlan, error) {
	errb := oops.In("intradaybar_router").With("provider", input.ProviderID, "market", input.Market, "security_type", input.SecurityType, "symbol", input.Symbol)
	...
	for _, candidate := range plan.Candidates {
		fetcher, ok := candidate.Impl.(Fetcher)
		if !ok {
			return RoutePlan{}, errb.With("provider", candidate.Provider.ID).New("routed intradaybar implementation does not satisfy Fetcher")
		}
		candidates = append(candidates, RouteCandidate{
			Provider: candidate.Provider,
			Group:    candidate.Profile.Group,
			Fetcher:  fetcher,
			Profile:  fetcher.IntradayBarProfile(),
