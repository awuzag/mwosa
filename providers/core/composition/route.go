...
type Router interface {
	RouteComposition(ctx context.Context, input RouteInput) (Lister, error)
	PlanComposition(ctx context.Context, input RouteInput) (RoutePlan, error)
}
...
func (r routeAdapter) RouteComposition(ctx context.Context, input RouteInput) (Lister, error) {
	errb := oops.In("composition_router").With("provider", input.ProviderID, "market", input.Market, "security_type", input.SecurityType, "symbol", input.Symbol)
	candidate, err := r.router.Route(ctx, toCoreRouteInput(input))
	if err != nil {
		return nil, errb.Wrap(err)
	}
	lister, ok := candidate.Impl.(Lister)
	if !ok {
		return nil, errb.With("provider", candidate.Provider.ID).New("routed composition implementation does not satisfy Lister")
	}
	return lister, nil
}

func (r routeAdapter) PlanComposition(ctx context.Context, input RouteInput) (RoutePlan, error) {
	errb := oops.In("composition_router").With("provider", input.ProviderID, "market", input.Market, "security_type", input.SecurityType, "symbol", input.Symbol)
	...
	for _, candidate := range plan.Candidates {
		lister, ok := candidate.Impl.(Lister)
		if !ok {
			return RoutePlan{}, errb.With("provider", candidate.Provider.ID).New("routed composition implementation does not satisfy Lister")
		}
		candidates = append(candidates, RouteCandidate{
			Provider: candidate.Provider,
			Group:    candidate.Profile.Group,
			Lister:   lister,
			Profile:  lister.CompositionProfile(),
