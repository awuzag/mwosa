...
type Router interface {
	RouteMarketTrades(ctx context.Context, input RouteInput) (Lister, error)
	PlanMarketTrades(ctx context.Context, input RouteInput) (RoutePlan, error)
}
...
func (r routeAdapter) RouteMarketTrades(ctx context.Context, input RouteInput) (Lister, error) {
	errb := oops.In("trades_router").With("provider", input.ProviderID, "market", input.Market, "security_type", input.SecurityType, "symbol", input.Symbol)
	...
	if !ok {
		return nil, errb.With("provider", candidate.Provider.ID).New("routed trades implementation does not satisfy Lister")
	}
	return lister, nil
}

func (r routeAdapter) PlanMarketTrades(ctx context.Context, input RouteInput) (RoutePlan, error) {
	errb := oops.In("trades_router").With("provider", input.ProviderID, "market", input.Market, "security_type", input.SecurityType, "symbol", input.Symbol)
	...
	for _, candidate := range plan.Candidates {
		lister, ok := candidate.Impl.(Lister)
		if !ok {
			return RoutePlan{}, errb.With("provider", candidate.Provider.ID).New("routed trades implementation does not satisfy Lister")
		}
		candidates = append(candidates, RouteCandidate{
			Provider: candidate.Provider,
			Group:    candidate.Profile.Group,
			Lister:   lister,
			Profile:  lister.TradesProfile(),
