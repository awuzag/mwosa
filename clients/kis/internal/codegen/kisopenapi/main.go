...
func queryParameters(api catalogAPI) []map[string]any {
	...
	for _, prop := range props {
		cli := cliMetadataForProperty(api, prop)
...
func cliExtension(cli cliMetadata) map[string]any {
	out := map[string]any{
		"flag":        cli.Flag,
		"source":      cli.Source,
		...
		"valueKind":   cli.ValueKind,
		"description": cli.Description,
		"advanced":    cli.Advanced,
	}
	if len(cli.Completion) > 0 {
		out["completion"] = cli.Completion
	}
	return out
}
...
func generatedConstants(files []splitFile) string {
	...
	buf.WriteString("\tOperationID string `json:\"operation_id\"`\n")
	buf.WriteString("\tEndpoint string `json:\"endpoint\"`\n")
	...
	buf.WriteString("\tGroup string `json:\"provider_group\"`\n")
	buf.WriteString("\tServiceGroup string `json:\"service_group\"`\n")
	buf.WriteString("\tRoleHint string `json:\"role_hint\"`\n")
	buf.WriteString("\tRealTRID string `json:\"real_tr_id\"`\n")
	buf.WriteString("\tVirtualTRID string `json:\"virtual_tr_id\"`\n")
	buf.WriteString("\tSupportsVirtual bool `json:\"supports_virtual\"`\n")
	buf.WriteString("\tParameters []ParameterMetadata `json:\"parameters\"`\n")
	buf.WriteString("}\n\n")
	buf.WriteString("type ParameterMetadata struct {\n")
	buf.WriteString("\tName string `json:\"name\"`\n")
	buf.WriteString("\tFieldName string `json:\"field_name\"`\n")
	buf.WriteString("\tFlag string `json:\"flag\"`\n")
	buf.WriteString("\tSource string `json:\"source\"`\n")
	buf.WriteString("\tRequired bool `json:\"required\"`\n")
	buf.WriteString("\tDefault string `json:\"default\"`\n")
	buf.WriteString("\tCanonical string `json:\"canonical\"`\n")
	buf.WriteString("\tValueKind string `json:\"value_kind\"`\n")
	buf.WriteString("\tDescription string `json:\"description\"`\n")
	buf.WriteString("\tAdvanced bool `json:\"advanced\"`\n")
	buf.WriteString("\tCompletion []string `json:\"completion,omitempty\"`\n")
	buf.WriteString("}\n\n")
	...
	buf.WriteString(")\n\n")
	buf.WriteString("type rawInvoker func(context.Context, Executor, map[string]string) (any, error)\n\n")
	buf.WriteString("var operationOrder = []string{\n")
	for _, file := range files {
		fmt.Fprintf(&buf, "\tOperation%s,\n", file.Config.RawFunction)
	}
	buf.WriteString("}\n\n")
	...
	for _, file := range files {
		...
		fmt.Fprintf(&buf, "\t\tSummary: %q,\n", cleanText(file.API.Name))
		fmt.Fprintf(&buf, "\t\tDescription: %q,\n", cleanText(firstNonEmpty(file.Config.DocSummary, file.API.APISummary, file.API.Description)))
		fmt.Fprintf(&buf, "\t\tGroup: Group%s,\n", suffix)
		fmt.Fprintf(&buf, "\t\tServiceGroup: ServiceGroup%s,\n", suffix)
		fmt.Fprintf(&buf, "\t\tRoleHint: %q,\n", file.Config.RoleCandidate)
		...
		buf.WriteString("\t\tParameters: []ParameterMetadata{\n")
		for _, prop := range filterProperties(file.API.Properties, "req_b", "") {
			writeParameterMetadata(&buf, file.API, prop)
		}
		buf.WriteString("\t\t},\n")
		buf.WriteString("\t},\n")
	}
	buf.WriteString("}\n\n")
	buf.WriteString("var operationInvokers = map[string]rawInvoker{\n")
	for _, file := range files {
		fmt.Fprintf(&buf, "\tOperation%s: invokeRaw%s,\n", file.Config.RawFunction, file.Config.RawFunction)
	}
	buf.WriteString("}\n\n")
	...
	buf.WriteString("\t\t}\n")
	buf.WriteString("\t\tmetadata.Parameters = cloneParameters(metadata.Parameters)\n")
	...
	buf.WriteString("\treturn LookupOperation(operationID)\n")
	buf.WriteString("}\n\n")
	buf.WriteString("func RequestTemplate(operationID string) (map[string]string, error) {\n")
	buf.WriteString("\tmetadata, ok := operationMetadata[operationID]\n")
	buf.WriteString("\tif !ok {\n")
	buf.WriteString("\t\treturn nil, fmt.Errorf(\"kis raw operation %q is not registered\", operationID)\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\ttemplate := make(map[string]string, len(metadata.Parameters))\n")
	buf.WriteString("\tfor _, parameter := range metadata.Parameters {\n")
	buf.WriteString("\t\ttemplate[parameter.Name] = parameter.Default\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\treturn template, nil\n")
	buf.WriteString("}\n\n")
	buf.WriteString("func Invoke(ctx context.Context, executor Executor, operationID string, input map[string]string) (any, error) {\n")
	buf.WriteString("\tinvoker, ok := operationInvokers[operationID]\n")
	buf.WriteString("\tif !ok {\n")
	buf.WriteString("\t\treturn nil, fmt.Errorf(\"kis raw operation %q is not registered\", operationID)\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\treturn invoker(ctx, executor, input)\n")
	buf.WriteString("}\n\n")
	buf.WriteString("func cloneParameters(parameters []ParameterMetadata) []ParameterMetadata {\n")
	buf.WriteString("\tout := make([]ParameterMetadata, len(parameters))\n")
	buf.WriteString("\tcopy(out, parameters)\n")
	buf.WriteString("\tfor i := range out {\n")
	buf.WriteString("\t\tout[i].Completion = append([]string(nil), out[i].Completion...)\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\treturn out\n")
	buf.WriteString("}\n\n")
	buf.WriteString("func applyInput(metadata OperationMetadata, input map[string]string) (map[string]string, error) {\n")
	buf.WriteString("\tknown := make(map[string]ParameterMetadata, len(metadata.Parameters))\n")
	buf.WriteString("\tout := make(map[string]string, len(metadata.Parameters))\n")
	buf.WriteString("\tfor _, parameter := range metadata.Parameters {\n")
	buf.WriteString("\t\tknown[parameter.Name] = parameter\n")
	buf.WriteString("\t\tif parameter.Default != \"\" {\n")
	buf.WriteString("\t\t\tout[parameter.Name] = normalizeInputValue(parameter, parameter.Default)\n")
	buf.WriteString("\t\t}\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\tfor key, value := range input {\n")
	buf.WriteString("\t\tparameter, ok := known[key]\n")
	buf.WriteString("\t\tif !ok {\n")
	buf.WriteString("\t\t\treturn nil, fmt.Errorf(\"kis raw operation %s does not accept parameter %s\", metadata.OperationID, key)\n")
	buf.WriteString("\t\t}\n")
	buf.WriteString("\t\tout[key] = normalizeInputValue(parameter, value)\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\tfor _, parameter := range metadata.Parameters {\n")
	buf.WriteString("\t\tif parameter.Required && strings.TrimSpace(out[parameter.Name]) == \"\" {\n")
	buf.WriteString("\t\t\treturn nil, fmt.Errorf(\"kis raw operation %s requires parameter %s (--%s)\", metadata.OperationID, parameter.Name, parameter.Flag)\n")
	buf.WriteString("\t\t}\n")
	buf.WriteString("\t\tif _, ok := out[parameter.Name]; !ok {\n")
	buf.WriteString("\t\t\tout[parameter.Name] = \"\"\n")
	buf.WriteString("\t\t}\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\treturn out, nil\n")
	buf.WriteString("}\n\n")
	buf.WriteString("func normalizeInputValue(parameter ParameterMetadata, value string) string {\n")
	buf.WriteString("\ttrimmed := strings.TrimSpace(value)\n")
	buf.WriteString("\tswitch parameter.ValueKind {\n")
	buf.WriteString("\tcase \"market\":\n")
	buf.WriteString("\t\tswitch strings.ToLower(trimmed) {\n")
	buf.WriteString("\t\tcase \"krx\":\n")
	buf.WriteString("\t\t\treturn \"J\"\n")
	buf.WriteString("\t\tcase \"nxt\":\n")
	buf.WriteString("\t\t\treturn \"NX\"\n")
	buf.WriteString("\t\tcase \"unified\", \"integrated\":\n")
	buf.WriteString("\t\t\treturn \"UN\"\n")
	buf.WriteString("\t\t}\n")
	buf.WriteString("\tcase \"period\":\n")
	buf.WriteString("\t\tswitch strings.ToLower(trimmed) {\n")
	buf.WriteString("\t\tcase \"daily\", \"day\", \"d\":\n")
	buf.WriteString("\t\t\treturn \"D\"\n")
	buf.WriteString("\t\tcase \"weekly\", \"week\", \"w\":\n")
	buf.WriteString("\t\t\treturn \"W\"\n")
	buf.WriteString("\t\tcase \"monthly\", \"month\", \"m\":\n")
	buf.WriteString("\t\t\treturn \"M\"\n")
	buf.WriteString("\t\tcase \"yearly\", \"year\", \"annual\", \"y\":\n")
	buf.WriteString("\t\t\treturn \"Y\"\n")
	buf.WriteString("\t\t}\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\treturn trimmed\n")
	buf.WriteString("}\n\n")
	for _, file := range files {
		writeRawInvoker(&buf, file)
	}
	return buf.String()
}

func writeParameterMetadata(buf *bytes.Buffer, api catalogAPI, prop catalogProperty) {
	cli := cliMetadataForProperty(api, prop)
	fmt.Fprintf(buf, "\t\t\t{Name: %q, FieldName: %q, Flag: %q, Source: %q, Required: %t, Default: %q, Canonical: %q, ValueKind: %q, Description: %q, Advanced: %t, Completion: %#v},\n",
		prop.PropertyCD,
		goName(prop.PropertyCD),
		cli.Flag,
		cli.Source,
		cli.Required,
		cli.Default,
...
func writeRawInvoker(buf *bytes.Buffer, file splitFile) {
	config := file.Config
	fmt.Fprintf(buf, "func invokeRaw%s(ctx context.Context, executor Executor, input map[string]string) (any, error) {\n", config.RawFunction)
	fmt.Fprintf(buf, "\tquery, err := applyInput(operationMetadata[Operation%s], input)\n", config.RawFunction)
	buf.WriteString("\tif err != nil {\n")
	buf.WriteString("\t\treturn nil, err\n")
	buf.WriteString("\t}\n")
	queryProps := filterProperties(file.API.Properties, "req_b", "")
	if len(queryProps) == 0 {
		buf.WriteString("\t_ = query\n")
	}
	fmt.Fprintf(buf, "\treturn %s(ctx, executor, %sRequest{\n", config.RawFunction, config.GoName)
	used := map[string]int{}
	for _, prop := range queryProps {
		fmt.Fprintf(buf, "\t\t%s: query[%q],\n", uniqueName(goName(prop.PropertyCD), used), prop.PropertyCD)
	}
	buf.WriteString("\t})\n")
	buf.WriteString("}\n\n")
}
...
func cliMetadataForProperty(api catalogAPI, prop catalogProperty) cliMetadata {
	field := strings.ToUpper(strings.TrimSpace(prop.PropertyCD))
	description := cleanText(firstNonEmpty(prop.Description, prop.PropertyName))
	meta := cliMetadata{
		Flag:        kebabName(field),
		Source:      "user",
		Required:    prop.RequireYN == "Y",
		Canonical:   "",
		ValueKind:   "string",
	...
	case "FID_INPUT_ISCD":
		...
		meta.ValueKind = "symbol"
	case "PDNO":
		meta.Flag = "product-no"
		meta.Canonical = "product_no"
		meta.ValueKind = "symbol"
	case "PRDT_TYPE_CD":
		meta.Flag = "product-type"
		meta.Canonical = "product_type"
		meta.Default = "300"
		meta.Source = "default"
		meta.Completion = []string{"300", "301", "302", "512", "513", "529"}
	case "FID_INPUT_DATE_1", "FID_INPUT_ST_DATE":
		meta.Flag = "from"
		meta.Canonical = "from"
		meta.ValueKind = "date"
	case "FID_INPUT_DATE_2", "FID_INPUT_ED_DATE":
		meta.Flag = "to"
		meta.Canonical = "to"
		meta.ValueKind = "date"
	case "FID_INPUT_DATE":
		...
		meta.Canonical = "date"
		meta.ValueKind = "date"
	case "FID_COND_MRKT_DIV_CODE", "FID_MRKT_CLS_CODE", "MRKT_DIV_CODE":
		meta.Flag = "market"
		meta.Canonical = "market"
		meta.ValueKind = "market"
		meta.Default = "J"
		meta.Source = "default"
		meta.Completion = []string{"krx", "nxt", "unified", "J", "NX", "UN"}
	case "FID_PERIOD_DIV_CODE":
		meta.Flag = "period"
		meta.Canonical = "period"
		meta.ValueKind = "period"
		meta.Default = "D"
		meta.Source = "default"
		meta.Completion = []string{"daily", "weekly", "monthly", "yearly", "D", "W", "M", "Y"}
	case "FID_ORG_ADJ_PRC":
		meta.Flag = "adjust-price"
		meta.Canonical = "adjust_price"
		meta.Default = "0"
		meta.Source = "default"
		meta.Completion = []string{"0", "1"}
		meta.Advanced = true
	case "CTX_AREA_FK100", "CTX_AREA_NK100":
		meta.Advanced = true
	case "FID_COND_SCR_DIV_CODE", "FID_RANK_SORT_CLS_CODE", "FID_INPUT_PRICE_1", "FID_INPUT_PRICE_2", "FID_VOL_CNT":
		meta.Advanced = true
	}
	if strings.Contains(strings.ToLower(api.AccessURL), "search-info") && field == "PDNO" {
		meta.Flag = "product-no"
	}
	if meta.Description == "" {
		meta.Description = cleanText(prop.PropertyName)
	}
	return meta
}
