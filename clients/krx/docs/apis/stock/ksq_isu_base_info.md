...
- 명세 endpoint: `https://data-dbg.krx.co.kr/svc/apis/sto/ksq_isu_base_info`
- 샘플 테스트 endpoint: `https://data-dbg.krx.co.kr/svc/sample/apis/sto/ksq_isu_base_info`
- 인증: HTTP request header `AUTH_KEY`에 사용자가 발급받은 인증키를 전달한다.
- 응답 형식: 상세 화면의 테스트 탭은 `json`과 `xml` 요청 타입을 제공한다.

```http
GET /svc/apis/sto/ksq_isu_base_info?basDd=20200414 HTTP/1.1
...
- 상세 페이지: <https://openapi.krx.co.kr/contents/OPP/USES/service/OPPUSES002_S2.cmd?BO_ID=CifLHplnUFMgpHIMMPXs>
- 서비스 목록: <https://openapi.krx.co.kr/contents/OPP/INFO/service/OPPINFO004.cmd>
- 개발 명세서 다운로드: `POST https://openapi.krx.co.kr/contents/OPP/USES/service/downloadApiDoc.cmd` with `path=sto`, `BO_ID=CifLHplnUFMgpHIMMPXs`, `BO_VER=1.0`
